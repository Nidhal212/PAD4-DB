...
# When called via `snakemake --use-conda`, the `snakemake` object is injected
# into this module's namespace. Override the output directories so Snakemake's
# declared outputs match what main() writes. BASE_DIR stays fixed (raw data).
if "snakemake" in dir():
