...
if "snakemake" in dir():
    _sm = snakemake  # noqa
    COMP_PARQ       = Path(_sm.input.compounds)
    OUT_CLIFFS      = Path(_sm.output.cliffs)
    OUT_PAIRS       = Path(_sm.output.pairs)
    OUT_SCAFFOLD    = Path(_sm.output.scaffold)
    OUT_CLIFF_JSON  = Path(_sm.output.cliff_json)
    OUT_PATENT_JSON = Path(_sm.output.patent_json)
