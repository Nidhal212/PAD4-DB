...
# 3-category source coloring: Multi-source / PubChem-only / Other single
...
# ── Panel A: Source combination (4 categories) ────────────────────────────────
...
ax_src.legend(handles=handles, loc='lower left', fontsize=5.0,
...
cb = fig.colorbar(sc, ax=ax_pic, pad=0.02, shrink=0.8, aspect=25)
...
mech_labels = {
    'enzymatic':           'Enzymatic (2,079)',
    'enzymatic_confirmed': 'Enzymatic confirmed (878)',
    'fp_ic50':             'FP-based IC50 (115)',
    'covalent':            'Covalent (21)',
}
for mech, clr in mech_colors.items():
    m = df['mechanism_class'] == mech
    ax_mec.scatter(x[m], y[m], s=MS, c=clr, alpha=ALF, linewidths=0,
                   rasterized=True, label=mech_labels[mech])
ax_mec.legend(loc='lower left', fontsize=4.8, frameon=False,
...
ax_hub.scatter(x[hub_mask_a], y[hub_mask_a], s=80, c=PAL['navy'],
               ...
               label='Hub A (n=2, 27 severe pairs)')
ax_hub.scatter(x[hub_mask_b], y[hub_mask_b], s=80, c=PAL['red'],
               marker='*', alpha=1.0, linewidths=0.3, edgecolors='white', zorder=5,
               label='Hub B (n=2, 23 severe pairs)')

# Labels with per-hub offsets to avoid overlap (B1/B2 are at nearly identical coords)
hub_label_specs = [
...
for ik, lbl, clr, (dx, dy) in hub_label_specs:
    ...
    if len(idx):
        ...
        ax_hub.annotate(lbl, (xi, yi), xytext=(xi + dx, yi + dy),
...
ax_hub.legend(loc='lower left', fontsize=4.8, frameon=False,
              handlelength=0.8, borderpad=0.2, labelspacing=0.2,
              markerscale=2)
