...
CLIFFS_PARQ = ROOT / "publication/data/activity_cliffs.parquet"
...
def phase_0a() -> int:
    ...
    # Phase 0A reads raw files before pipeline exclusions; the pipeline ingests 341,282
    # after excluding 46 dual-layer secondary copies (AIDs 1920046/2202442 ×23 each).
    # MASTER_AUDIT Phase 0A reads 341,295 rows (a counting-method artifact documented
    # in CLAUDE.md; see DROPPED_ROWS_13.txt). The pipeline ingestion count of 341,282
    # is verified in Phase 1. Tolerance here covers the 13-row artifact.
    claimed = 341_295
    diff = abs(total - claimed)
    print(f"\n  TOTAL: actual={total:,}  claimed={claimed:,}  diff={diff}")
    if diff > 20:
        stop(f"Phase 0A: raw row total {total:,} ≠ {claimed:,} (diff={diff})")
    vt("Total raw rows", claimed, total, tol=20, exact=False)
    return total
...
def phase_0b() -> int:
    ...
    claimed_active = 95  # 97 files total; 2 AIDs (1920046, 2202442) each span 2 subdirs → 95 unique
...
def phase_3():
    ...
    print(f"    Multi-replicate groups: {len(multi)} (expected 450)")
    vt("Multi-replicate groups", 450, len(multi))
...
def phase_5(comp: pd.DataFrame):
    ...
    # 5E Hub compound verification — count SEVERE cliff pairs only
    # (CLAUDE.md records severe-tier hub pair counts: 12 for B1, 11 for B2)
    ...
    for hub_ik, expected_n, label in [
        ...
        n_pairs = int(
            ((severe_df["inchi_key_a"] == hub_ik) | (severe_df["inchi_key_b"] == hub_ik)).sum()
        )
        # Also try by prefix (first 14 chars) if full IK not found
        prefix = hub_ik.split("-")[0]
        if n_pairs == 0:
            n_pairs = int(
                (severe_df["inchi_key_a"].str.startswith(prefix) |
                 severe_df["inchi_key_b"].str.startswith(prefix)).sum()
            )
            if n_pairs > 0:
...
def phase_7(comp: pd.DataFrame):
    ...
    present_not_mapped = {
        ...
        "BB-Cl-Amidine":  "YLSHDLCSANAKPH-QHCPKHFHSA-N",
    }
