...
if "snakemake" in dir():
    _sm = snakemake  # noqa
    POT_PATH  = Path(_sm.input.potency)
    HTS_PATH  = Path(_sm.input.hts)
    STD_PATH  = Path(_sm.input.standardized)
    NORM_PATH = Path(_sm.input.normalized)
    OUT_AID   = Path(_sm.output.aid_level)
    OUT_COMP  = Path(_sm.output.compounds)
    OUT_HTS   = Path(_sm.output.hts_index)
